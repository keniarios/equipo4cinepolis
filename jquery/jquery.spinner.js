...
;(function ($) {
  $.fn.spinner = function (opts) {
    return this.each(function () {
      var defaults = {value:0, min:0, max:10}
     ...
     function changeValue(delta) {
        //funcion entera modificada (MJO-Tom�s)
        var index1 = (textField[0].id.lastIndexOf("_"))
        var idCombo = textField[0].id.substring(0,index1) + "_ddlQty" + String(textField[0].id.substring(index1-1,index1)-1)
        var combo = document.getElementById(idCombo)
        if ((getValue() + delta) < combo.options.length){
            textField.val(getValue() + delta)
            combo.selectedIndex = getValue()
            combo.onchange()
